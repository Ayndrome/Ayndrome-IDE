...
// src/server/sandbox/terminal-server.ts
// Updated signature — accepts pre-created noServer WSS
// so upgrade routing is controlled by server.ts
...
import type { Server } from "http";
import { getOrCreateSandbox } from "./sandbox-manager";
import type { ClientMessage, ServerMessage } from "./terminal-protocol";
...
// ── Updated: accepts existing WSS instead of creating one ────────────────────
export function createTerminalWSS(
    _httpServer: Server,           // kept for API compat, no longer used directly
    wss: WebSocketServer,          // noServer WSS from server.ts
): WebSocketServer {
    ...
    wss.on("connection", async (ws: WebSocket, req: IncomingMessage) => {
        ...
        try {
            ...
            ptyProcess.onExit(({ exitCode }) => {
                console.log(`[Terminal] PTY exited: ${sessionId} code=${exitCode}`);
                send(ws, { type: "exit", code: exitCode ?? 0 });
                ptySessions.delete(sessionId);
                ws.close();
            });
            ...
            ws.on("message", (raw: Buffer) => {
                try {
                    ...
                    if (msg.type === "resize") ptyProcess.resize(
                        Math.max(1, msg.cols),
                        Math.max(1, msg.rows)
                    );
            ...
            console.error(`[Terminal] Session failed:`, err.message);
            send(ws, { type: "error", message: `Failed: ${err.message}` });
            ws.close();
        }
    });
